...
Apply routine clarification/prioritization within the user's triage authorization.
Ask for decisions that change scope, cancel work or select deletion when not
already authorized. Never silently delete duplicates; retain history and link
the canonical task. Update the authoritative system, reread external mutations,
and record reasoning/next action in workspace continuity. No local external
status copy, no implementation or auto-build from triage alone.
