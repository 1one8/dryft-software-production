---
name: dryft-software-production-add
...
First invoke core `/dryft-add-project` (Codex may use its skill picker or `$`),
following its exact registration/inspection rules. Its CLI is
`dryft --workspace WORKSPACE project add PROJECT`; registration changes only
...
Detect `.dryft/project.json` and its brief reference. An unrelated `.dryft/` or
