...
Use the supplied overview; ask for it if missing. Read the brief and authoritative
tasks to avoid duplicate work. Derive title, problem/change, scope, acceptance,
dependencies and verification from evidence; use task interview only for gaps.
...
its returned ID/URL only after success. Never claim creation from a preview or
invent an ID. Preserve external descriptions/status as sole authority; an optional
local context note uses the external-reference template, not a copied task.
Record the reference in workspace continuity. If brief is absent, allow a small
explicitly requested maintenance task with that gap recorded; do not force a new
project interview for a bounded change.
